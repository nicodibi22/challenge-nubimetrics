...
namespace challenge_nubimetrics_services.Implementations
{
    public class PaisesImplementation : PaisesService
    {
        ...
        public PaisesImplementation()
        {
            ...
            {
                cfg.CreateMap<CountryML, PaisDTO>()
                ...
                cfg.CreateMap<GeoInformationML, GeoInformacionDTO>()
                    .ForPath(x => x.Locacion, opt => opt.MapFrom(y => y.Location));
                cfg.CreateMap<StateML, EstadoDTO>()
                    .ForMember(x => x.Nombre, opt => opt.MapFrom(y => y.Name));
                cfg.CreateMap<LocationML, LocacionDTO>()
        ...
        private async Task<PaisDTO> GetPaisInfoFromApi(string pais)
        {
            ...
            string parameters = "";
            return _mapper.Map<CountryML, PaisDTO>(await RestApiCaller.GetRequest <CountryML>(url, parameters));
        }
    }
}
